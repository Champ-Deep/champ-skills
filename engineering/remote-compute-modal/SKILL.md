...
description: Run GPU jobs on the user's own Modal account via host.compute.create('modal', provider_params={...}) — the create→submit→wait_for_notification flow, the compute_provider kernel for env setup, image/volume resolution, and the two approval cards. Load once you've decided to dispatch to Modal.
license: Apache-2.0
---

You're dispatching to the user's Modal account: containers spin up in Modal's
cloud, on hardware you name in plain terms (gpu/cpu/memory), under
...
`host.compute.create('modal', ...)` resolves the target the moment you call
it: if Modal isn't enabled in this install it raises `host.compute.NotFound`
listing the targets that do exist — ask the user to enable it under
*Settings → Compute → Modal* (or surface the prompt; you can't enable it for
them). A bad `provider_params` key or value raises
`host.compute.InvalidResource` naming the field, before anything is staged
or billed.
...
## Clocks and container lifetime

You set exactly one clock: `run_timeout_s` on `submit_job`, the job's own
wall-clock guard, in integer seconds. Everything else is either derived or
a platform constant you're told about, not a knob you turn.

1. Container life starts at sandbox creation, on the first `submit_job()`
   on a handle — staging the inputs counts against it, so a big upload
   spends container time before the job runs. The container's wall clock
   is Modal's `Sandbox.create(timeout=)`, filled from the Settings default
   for this provider (*Settings → Compute → Modal*) unless you pin it via
   the `timeout` key in `provider_params` (ceiling: Modal's 24 h platform
   lifetime minus the staging margins — 85,500 s).
2. The job runs. If it exceeds `run_timeout_s` it is TERMed there.
   Omitted, `run_timeout_s` defaults to the container's remaining life
   minus the harvest margin, and `job.notes` records the value that was
   filled — read it back as `job.deadline` / `job.time_left_s()`. Name one
   when you know the job's budget: a hung job then costs that budget, not
   the whole container, and the warm container survives for the next
   submit.
...
5. Whatever is under `./out/` (plus the logs) is tarred and staged
   ...
   `harvest_failed`), the staged copy waits on the sandbox — see "When the
   job fails".

After a job finishes, the sandbox idles **warm for 15 minutes** waiting for
your next submit on the same handle, then its PID-1 **idle watchdog**
self-terminates it. That window is a platform constant, disclosed in
`job.notes` / `res.notes` and in `host.compute.ledger()`'s sandbox rows
(`auto_terminate_in_s`) — never a kwarg. Your two lifetime actions are
"submit again on the same handle" (stays warm) and `c.close(intent=...)`
(terminate now).

A deadline-ended job lands as `state == 'timed_out'` — not a generic
...
a larger `run_timeout_s`, or resuming from a harvested checkpoint. A
...
`host.compute.create('modal', provider_params={...})` is the **job
surface**. It runs in the `repl` tool — same as `remote-compute-ssh` — and
is what you use for anything with inputs/outputs you want harvested into
the workspace, anything on a GPU, and anything long enough that you'd want
to reattach if the daemon restarts. `create()` validates the target and the
`provider_params` immediately, but the **tier card** (provider, image,
`1× A100-40GB · 8 CPU · 32 GB`, container timeout, volumes) and the actual
Sandbox creation both happen on the first `submit_job()`.
`submit_job` / `attach_job` / `job.result()` / `close` work as for SSH;
the ssh-only verbs (`call_command`, `download`, `upload`) raise
`host.compute.NotSupported` on this handle. `job.result()` never waits —
waiting is the daemon poller's job: it probes the sandbox, harvests
`out.tar.gz` into `hpc/<jobId>/`, and posts a `compute_done` notification
when done. A cell that blocked on the job instead would tie up your
control-plane kernel for a GPU run's whole duration, so calling `.result()`
before the notification raises `host.compute.JobPending`, whose `next_step`
is the park protocol. To wait, exit the cell after `submit_job` and use the
`wait_for_notification` brain-tool. What reaches the remote is
what was in the workspace files you named, and what comes back lands under
`hpc/<jobId>/` through the same hardened extractor.
...
prompts. The kernel rejects `gpu=` on `Sandbox.create`, and rejects any
`timeout` above 1800 s (30 min) — loudly, at create time; nothing is
silently clamped. That cap is the kernel's own probe budget, separate
from the job surface's container timeout, and it is a real wall: a
sandbox you create here and are still using dies at its `timeout`
(outputs inside it are then unrecoverable), so size each staging/probe
step to fit, checkpoint to a Volume, or move long work to the job
surface. An unset `timeout` keeps the SDK default (300 s). It's a
redirect to the job surface, not a fence; you've already been handed
the credential. **Terminate every sandbox you create here** —
`sb.terminate()` in the same cell, in a `try/finally` if you `exec` in
between (`build_env`'s hydrate models this). Nothing reaps it
mid-session: the idle timeout kills the *kernel*, never your sandboxes —
the ≤30-min cap only bounds how long a forgotten one bills.

A Modal workspace is commonly shared across many Claude Science
installs. `modal.Sandbox.list()` without `app_id=` returns every
live sandbox in the workspace — other users' running GPU jobs
alongside yours — so hundreds of unfamiliar sandboxes is normal
load, not a leak, and terminating one destroys another team's work.
Terminate only sandbox ids this session's own `Sandbox.create()`
returned; never enumerate-then-terminate.
...
Start with `compute_details({provider: 'modal', mode: 'read'})`. Below
...
control-plane kernel (a `repl` cell) and pass it as
`provider_params['image']` — and, for a bundled env, its NAME as
`provider_params['env']`: the host then reads the env's
...
`compute_provider` cell entirely. **Pass `r['image']` (the `im-…` ref) as
`provider_params['image']`.** The adapter will resolve a bare env name
...
`provider_params` is ONE flat dict in Modal's own vocabulary — the accepted
keys are `image`, `env`, `gpu`, `cpu`, `memory`, `volumes`, `timeout` — and
it is validated when `create()` runs, so a typo raises there rather than
after the inputs are staged. For a quick inline call (no inputs, output is
just stdout), use `submit_job` with no `inputs`. The `compute_done`
notification payload carries `state` / `exit_code` / `notes` /
`output_files`, plus `error_kind`, `system_hint`, and `deadline_fired`
when set — read those disclosures from the notification itself. For
`stdout_tail`, re-enter the kernel and read it off `.result()` after the
...
j = c.submit_job(
    ...
    run_timeout_s=300,             # this probe finishes in 5 min or it's hung
)
# the Job repr just printed itself: id, notes (cold start, deadline), the
# re-attach line — the cell ends here, the kernel never waits on the job
```

Then exit the cell and use `wait_for_notification`. When the
`compute_done` notification arrives, its payload has `state` /
`exit_code` / `notes` / `output_files`, plus `error_kind` /
`system_hint` / `deadline_fired` when set. For `stdout_tail`, re-enter the
kernel:

```python
# repl tool — cell ② after the compute_done notification
res = c.attach_job('<JOB_ID>').result()   # a read, not a wait — the poller already collected ./out/
print(res.stdout_tail)
c.close(intent='probe done — stop the sandbox')
```
...
c = host.compute.create('modal', provider_params={
    ...
    'timeout': 3600,                     # container wall clock, seconds — optional; omitted → Settings default; ceiling: Modal's 24 h platform limit
})
# → the tier card renders on the first submit_job → Sandbox.create() under the user's workspace
...
job = c.submit_job(
    ...
    inputs=['target.fasta'],                     # bare string ≡ {'src': 'target.fasta'} → ./target.fasta
    outputs=['ranked_0.pdb', '*.json'],   # only these come back from ./out/ → output_files; a *.log your tool writes under out/ is NOT kept unless you name it (the run's own stdout.log/stderr.log always land, as logs)
    run_timeout_s=2700,                  # job guard — optional; default = remaining container life − harvest margin
)
# repr printed itself (id, notes, deadline, egress); cell ends here
```

Then `wait_for_notification` — the `compute_done` payload includes
`output_files`, so you can `save_artifacts(payload['output_files'])`
directly without re-entering the kernel. If you want the full record
(`stdout_tail` / `stderr_tail` / `wall_s` / `notes`):

```python
# repl tool — cell ② after the compute_done notification
res = c.attach_job('<JOB_ID>').result()   # read of the persisted result — never a wait
save_artifacts(res.files)
c.close(intent='task done — stop the sandbox')
```

`res.wall_s` (also `res["job_wall_s"]`) is the seconds `run.sh` ran on
the remote — the actual GPU/compute duration.

`inputs=` entries are `{src, dst}` (a bare `'path'` string is `{'src': 'path'}`)
and stage **flat** into the workdir root: `src` is a workspace-relative path
(or `artifact://<version_or_artifact_id>` for an artifact-store file — `dst`
required then); `dst` is a bare filename — `'inputs/gfp.fasta'` is rejected
at submit (the file would land at `/work/gfp.fasta`, not where the command
expects). `src` can be a path; `dst` can't. Omit `dst` to default to
`basename(src)`. Need a dir layout? `mkdir -p` it inside `command=`.
...
per-submit cap** (a `TransferTooLarge` refusal above it) and its **64
files per-submit cap** (every input is held open at once for the
streaming upload; a 65th is refused before anything stages — tar many
small files into one archive and unpack it in `command=`) — so beyond
either a Volume is the only path, and the better one well before that. Volumes
(Files-tab import, or a `compute_provider` cell; mount via
`provider_params['volumes']`) persist on Modal disks across jobs and
...
**Only `./out/` (+ the logs) comes back — and that's the whole point of the
convention.** A Modal sandbox is destroyed with its filesystem, so unlike
ssh there is no persistent workdir to leave things in or chain from; the
one durable move is the harvest, and the wrapper makes it a single bounded
stream by tarring exactly `out/` + `stdout.log` + `stderr.log`. Everything
else in the sandbox is gone at teardown. `outputs=` globs select within
that scope — name `['ranked_0.pdb', '*.json']` and only those files under
`./out/` come back (`output_files`); omit `outputs` and all of `./out/`
comes back — and `exclude=[...]` subtracts globs the same way (a checkpoint
dir you don't want back). The run's own `stdout.log`/`stderr.log` are not
in `output_files` — they're the run's logs, not files you named; they land
beside the deliverables in `hpc/<job_id>/` and feed `stdout_tail`. Because the tarball is built sandbox-side before
either list applies, on byoc they shape `output_files`, not the bytes on the
wire; for the same reason there are no per-file `transfer_limits` here (it
raises `NotSupported`) — one fixed whole-stream cap governs the tarball, so
you bound what comes back by what you write under `./out/`. If your tool
writes to cwd, a Volume mount (e.g. `/atlas`), or `$HOME`, end `command=`
with `cp -r <results> out/` (or point the tool's `-o`/`--output` at
`out/`). A successful job with an empty `out/` returns no output files
plus a note telling you so (the logs still land beside them).
...
token` failures; when you must interpolate a value, `shlex.quote()` it.

`provider_params` uses **Modal's own kwarg names** — Modal's docs are the
docs. The adapter resolves the strings to SDK objects (env name → its
...
platform cap, out-of-range hardware, or an unknown key is rejected at
`create()` with `InvalidResource` — nothing is silently clamped, and the
family-nested `{'modal': {...}}` wrapper from older transcripts is
accepted only as a mute alias for the flat dict.
...
closed on a foreign or fabricated id, so a typo surfaces as `NotFound`
...
Measured against real Modal during validation — set clocks and user-status
...
| `compute.create()` (target lookup + params check) | one host round-trip | no container, no card |
...
Cold starts and reuse both announce themselves in `job.notes`.
...
`next_step` says which case you're in. Size the fan-out from
`c.concurrency` (`Slots(live, limit)`) up front so sub-agents rarely hit
the cap. Sub-agent code is otherwise unchanged.
...
submits past the provider's own ceiling raise a `Busy` error instead of
...
subsequent calls on the same `c` reuse it warm (weights/XLA cache stay
hot). The poller's harvest does **not** terminate the container — it runs
until `c.close(intent=...)`, 15 minutes of inactivity (the sandbox's
PID-1 **idle watchdog** — a kill, not a harvest: anything unharvested
goes with the container), or its container timeout, whichever comes
first. Until then a finished — or failed — job's container bills idle,
and both the countdown and its remedy are in `job.notes` and
`host.compute.ledger()`. **Every handle ends with
...
sequence ends. `close()` is host-scoped, so it also reaches a warm
sandbox from a *recreated* handle after a kernel reap
(`host.compute.create('modal').close(intent=...)`), and it returns a
`CloseReport` naming what it actually terminated. Near end of life the
**harvest watchdog** stops any running job early enough to stage its
outputs, so even the last job comes back harvested. A warm container is
reused only when its remaining life covers the next job's deadline plus
margin; otherwise the submit creates a fresh sandbox (one more cold
start, same handle) and says so in `job.notes` — the swap is never
silent.
...
detached wrapper survives a Claude Science daemon restart — on restart
you'll see "1 job reattached" rather than a cold start.
...
`{status:'error'}` (= none left). There's no need to poll `job.state()`
/ `.result()` — the notifications arrive on their own, and each poll is a
live host round-trip that tells you nothing the wake-up wouldn't.

**Before you finish the task**: every handle closed (unless the user
asked to keep one warm, or its sandbox holds unrecovered
`result_rejected`/`harvest_failed` outputs). Tear down job-surface
containers with `c.close()`, never a raw `sb.terminate()` — only
`close()` consults the host's recoverable-outputs gate. Terminate a
kernel probe sandbox only by the id you hold from creating it — the
workspace is shared and the app can be too, so an enumerate-then-
terminate sweep destroys other teams' running work.
`host.compute.ledger()` is the cheap final check that nothing you own is
still live.
...
watchdog terminates it after 15 minutes of inactivity, and reading files
...
loses only what didn't come back (a core dump, a verbose log, a file
outside `./out/` or left out of `outputs`) — and `c.download()` can't save
it here:
it is ssh-only and raises `NotSupported` on this handle. Read any such
file off the live sandbox from a `compute_provider` cell BEFORE you
close: find it with `modal.Sandbox.list(app_id=app.app_id,
...
in `job.notes`) — then `sb.exec("cat", "<path>")` to pull the bytes.

Read `res.exit_code`, `res.stdout_tail`, and `res["stderr_tail"]` (and
`res.notes`, which repeat any host-authored remedy hint). A job
that hit a deadline lands as `state == 'timed_out'` with its partial
outputs already harvested into `hpc/<job_id>/` — check `stdout_tail` for
progress and resubmit with a larger `run_timeout_s` (or resume from a
harvested checkpoint) only if the work was actually cut short. Failures
that never produced an exit code raise instead: every leaf under
`host.compute.Error` carries a closed `.kind`, and a submit-time or
provider failure arrives as `ProviderError` whose `.kind` (the same
string the `compute_done` payload's `error_kind` carries) maps cleanly
onto where to look:
...
(surfaces as a `Busy` error). For the concurrent limit, first free what
you hold: `c.close()` finished
handles (not one holding unrecovered outputs — see below) and terminate
kernel sandboxes you created and are done with — idle warm containers
count against it. The limit is workspace-shared: never terminate a
sandbox you didn't create to free capacity. A spend cap isn't freed by
closing — accrued spend stays accrued. The workspace's deployed-apps
cap also lands here: closing sandboxes doesn't free it either — the
user has to stop stale apps (dashboard, or `modal app stop`; shared
workspace, so never stop apps you didn't create), and reusing the
bound/configured app avoids refilling it. If
it still trips, surface the copy verbatim and pause; raising the limit
is the user's billing decision.

`rate_limited` — a request-rate throttle, not capacity (`.retryable`
is true): the token's request rate, or a Volume control-plane limit
(`rate limit exceeded` on listings, `too many layers` while Modal
compacts a volume in the background). Closing containers frees nothing
here. Back off ~60s and stagger fan-out submissions / batch volume
listings, per the error text — don't hand-roll a tight retry loop.

`ownership_mismatch` — another install sharing this workspace token adopted
the warm sandbox first. The adapter cold-starts a fresh one and says so in
`job.notes`; nothing for you to do beyond noting it.
...
after 15 minutes of inactivity, only copy included.
...
yours: they retrieve from the recovery list in the Compute panel. Same
clocks as above: surface it immediately.
...
inputs — same diagnosis as on SSH: read the tails, `res.raise_for_status()`
if you want the typed `JobFailed`, fix, resubmit.
...
Pass `'volumes': {"/weights": "<vol-name>"}` in `provider_params`. The
mount is a symlink to `/__modal/volumes/<id>/` — always reference it by the
mount path, never the resolved target.

The Files tab lists the Modal target alongside SSH hosts: at `/` it shows
...
`provider_params` key (the schema rejects `environment` /
...
`provider_params['env']` (or let the `### env:` ledger block name
...
At submit time the reply carries an `egress` line — the sandbox's birth
policy — which the Job holds as `job.egress`, e.g. `allowlist (2
domains; mirror off; +0 from the env)`. That line describes the sandbox
for its whole
...
carrying connection-level failures and the sandbox was fenced, the result's
`notes` and the `compute_done` notification carry a second host-authored
line starting with `[egress]` that names the policy, its size, and the
remedy. Both lines are written by the host from the policy it actually
enforced — they are evidence you can repeat to the user verbatim, not
...
The fix depends on the failure class, and the `[egress]` note names the
...
failure you already have; the `[egress]` note and `job.egress` already
...
- **`Volume.from_name(...)` returns a lazy handle** — `vol.object_id` raises `AttributeError: unhydrated` until the volume is mounted into a `Sandbox.create(volumes={...})`. Don't try to read it; record the volume *name* (that's what `provider_params['volumes']` takes anyway).
